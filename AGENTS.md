...
- `src/components/AnimatedIcons/*.tsx` + `src/animated-icons.ts` - **AUTO-GENERATED — DO NOT EDIT.** Generated by `pnpm icons:animated` from the map in `scripts/animated-icons.map.json`. See `src/components/AnimatedIcons/NOTICE.md`; `types.ts` and `useIconAnimation.ts` are hand-written.
...
render in the **same box**, since consumers swap it in by changing the import
path alone. That contract is enforced two ways: both sets share
`ICON_SIZE_CLASS`, and the generated tests render each animated icon beside its
static twin and compare box classes.

To add one, put `"OurIconName": "registry-slug"` in
`scripts/animated-icons.map.json` and run `pnpm icons:animated`. Only map a slug
whose artwork is the **same glyph** as ours — the `review` block in that file
lists pairs that were rejected for differing (filled vs outline, mirrored, extra
elements) and why.
