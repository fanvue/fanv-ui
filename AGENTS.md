...
- `src/styles/theme.css` - **AUTO-GENERATED — DO NOT EDIT.** Generated by `node src/styles/buildStyles.js` from `styleTokens.json`. Any manual changes will be overwritten on next regeneration.
- `src/styles/base.css` - Hand-written styles, utilities, and manual design tokens. This is where all non-generated CSS belongs (imported by `theme.css`).
- `src/styles/buildStyles.js` - Script that generates `theme.css`
- `src/utils/cn.ts` - className utility
- `src/components/Icons/*.tsx` - **AUTO-GENERATED — DO NOT EDIT.** Generated from Figma exports by `pnpm icons:generate`. `BaseIcon.tsx`, `types.ts`, and `iconSizeClass.ts` are hand-written.
- `src/components/AnimatedIcons/*.tsx` + `src/animated-icons.ts` - **AUTO-GENERATED — DO NOT EDIT.** Generated by `pnpm icons:animated` from the map in `scripts/animated-icons.map.json`. See `src/components/AnimatedIcons/NOTICE.md`; `types.ts` and `useIconAnimation.ts` are hand-written. Licence texts for the imported artwork live in `THIRD-PARTY-NOTICES.md`, which ships in the package.
- `src/docs/Icons.stories.tsx` - **AUTO-GENERATED — DO NOT EDIT.** Generated by `scripts/generate-icon-stories.mjs`; change the generator instead.
...
# Icons (all generated — edit the scripts, never the output)
pnpm icons:sync       # Re-import from Figma, then regenerate components/tests/stories
pnpm icons:animated   # Re-import animated twins, then regenerate tests/stories
```

### Icons

Two generated sets, both driven from `scripts/`:

- **Static icons** come from Figma. `pnpm icons:sync` writes
  `src/components/Icons/`, `scripts/icons.manifest.json`, the tests, the story,
  and the export block in `src/index.ts`.
- **Animated icons** come from the [lucide-animated](https://lucide-animated.com)
  registry (MIT), on the optional `@fanvue/ui/animated-icons` subpath with
  `motion` as an optional peer — the same pattern as `./charts` + `recharts`, so
  the main entry stays free of Motion.

An animated icon is published under the **same name** as its static twin and must
be a drop-in for it, since consumers swap it in by changing the import path
...
- **It actually animates.** Upstream does not name its variants consistently
  ...
  labels therefore travel with each icon, the importer checks them against the
  variant tables it emits, and every generated icon is hovered in a test and
  asserted to move.

Nothing animates under `prefers-reduced-motion` — the gate lives in
`useIconAnimation`, which every generated icon routes through.

To add one, put `"OurIconName": "registry-slug"` in
`scripts/animated-icons.map.json` and run `pnpm icons:animated`. Only map a slug
whose artwork is the **same glyph** as ours, at the same fill/stroke treatment —
the `review` block in that file lists pairs that were rejected (filled vs
outline, mirrored, extra elements) and why. The registry responses are committed
under `scripts/animated-registry/` and hash-verified on every run, so a
regeneration is offline and reproducible; `--refresh` re-fetches. A run in which
any icon fails writes nothing at all.
