...
| --- | --- | --- |
| `@fanvue/ui` | The component library: buttons, inputs, overlays, layout, avatars, chat, tables, 173 static icons, plus the `cn` helper. | None |
...
| `@fanvue/ui/flags` | `CountryFlag` and the `CountryFlagCode` union. | None |
| `@fanvue/ui/animated-icons` | 69 animated twins of static icons. | `motion@^12 \|\| ^13` |
...
**Size budgets in CI.** Two entry points are size-checked on every pull request with `size-limit`: `dist/index.mjs` has a 220 KB budget and `dist/animated-icons.mjs` has a 15 KB budget, both measured with external dependencies excluded. A change that pushes either past its budget fails the build.
...
```tsx
import { CountryFlag, type CountryFlagCode } from "@fanvue/ui/flags";

<CountryFlag code="gb" size={24} />;
```

Codes are mostly ISO 3166-1 alpha-2, plus the extras the artwork source ships (`eu`, `un`, `xk`, and others). `CountryFlagCode` is the exhaustive union, so an unknown code is a type error rather than a blank space at runtime.
...
Storybook's **Foundations → Icons** page lists the full set with **animated** switched on: icons badged `anim` have a twin, and clicking a card copies the correct import.
