...
import { PauseBarsIcon } from "../Icons/PauseBarsIcon";
...
describe("AudioPlayer", () => {
  describe("idle rendering", () => {
    ...
    it("keeps the elapsed slot rendered but hidden, so the timer does not resize on playback", () => {
      render(<AudioPlayer src="https://example.com/clip.mp3" duration={5} />);
      expect(screen.getByText("/0:05")).toHaveClass("invisible");
    });
  ...
  describe("play/pause", () => {
    it("renders the bare play triangle, not the circled play glyph", () => {
      render(<AudioPlayer src="https://example.com/clip.mp3" duration={5} />);
      const { container: bareGlyph } = render(<PlayTriangleIcon />);
    ...
    it("renders the bare pause bars, not the circled pause glyph", async () => {
      ...
      const { container: bareGlyph } = render(<PauseBarsIcon />);
  ...
  describe("progress", () => {
    it("switches to the elapsed/total format once playback has progressed", () => {
      ...
      expect(screen.getByText("/0:05")).not.toHaveClass("invisible");
