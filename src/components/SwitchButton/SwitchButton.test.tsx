...
describe("SwitchButton", () => {
  describe("API", () => {
    ...
    it("respects defaultPressed when uncontrolled", () => {
      render(<SwitchButton label="Filter" defaultPressed />);
      const button = screen.getByRole("button");
    ...
    it("can be controlled", () => {
      const { rerender } = render(<SwitchButton label="Filter" pressed={false} />);
      const button = screen.getByRole("button");
      expect(button).toHaveAttribute("aria-pressed", "false");

      rerender(<SwitchButton label="Filter" pressed={true} />);
      expect(button).toHaveAttribute("aria-pressed", "true");
      expect(button).toHaveAttribute("data-state", "on");
    });
    ...
    it("does not move a controlled value on its own", async () => {
      ...
      render(<SwitchButton label="Filter" pressed={false} onPressedChange={onPressedChange} />);
      const button = screen.getByRole("button");
    ...
    it("toggles on click and reports the next state", async () => {
      const user = userEvent.setup();
      const onPressedChange = vi.fn();
      render(<SwitchButton label="Filter" onPressedChange={onPressedChange} />);
      const button = screen.getByRole("button");
    ...
    it("still calls a supplied onClick", async () => {
      const user = userEvent.setup();
      const onClick = vi.fn();
      render(<SwitchButton label="Filter" onClick={onClick} />);
  ...
  describe("size", () => {
    it.each([
      ...
      render(<SwitchButton label="Filter" size={size} />);
  ...
  describe("icons", () => {
    ...
    it("shows neither glyph by default", () => {
      const { container } = render(<SwitchButton label="Filter" />);
      expect(glyphCount(container)).toBe(0);
    });

    it("toggles the two glyphs independently", () => {
      const { container, rerender } = render(<SwitchButton label="Filter" showLeftIcon />);
      expect(glyphCount(container)).toBe(1);

      rerender(<SwitchButton label="Filter" showRightIcon />);
      expect(glyphCount(container)).toBe(1);

      rerender(<SwitchButton label="Filter" showLeftIcon showRightIcon />);
      expect(glyphCount(container)).toBe(2);

      rerender(<SwitchButton label="Filter" />);
      expect(glyphCount(container)).toBe(0);
    });

    it("puts the glyphs on the correct sides of the label", () => {
      const { container } = render(<SwitchButton label="Filter" showLeftIcon showRightIcon />);
    ...
    it("keeps glyphs out of the accessible name", () => {
      render(<SwitchButton label="Earnings" showLeftIcon showRightIcon />);
  ...
  describe("variant", () => {
    it("pins a single fixed sparkle on the ai variant", () => {
      const { container } = render(<SwitchButton variant="ai" label="Smart replies" />);
      const children = Array.from(
        (container.firstElementChild as HTMLElement).children,
      ) as HTMLElement[];
    ...
    it("fills green and paints a masked three-stop gradient ring when pressed", () => {
      render(<SwitchButton variant="ai" label="Smart replies" defaultPressed />);
      const button = screen.getByRole("button");
    ...
    it("carries no ai fill while unpressed", () => {
      render(<SwitchButton variant="ai" label="Smart replies" />);
  ...
  describe("disabled", () => {
    it("does not toggle when disabled", async () => {
      const user = userEvent.setup();
      const onPressedChange = vi.fn();
      render(<SwitchButton label="Filter" disabled onPressedChange={onPressedChange} />);
      const button = screen.getByRole("button");
    ...
    it("drops the pressed fill so the disabled treatment reads as unavailable", () => {
      render(<SwitchButton label="Filter" disabled defaultPressed />);
      const button = screen.getByRole("button");
      expect(button).toHaveAttribute("aria-pressed", "true");
  ...
  describe("accessibility", () => {
    it("has no accessibility violations", async () => {
      const { container } = render(<SwitchButton label="Hide sold out" />);
      expect(await axe(container)).toHaveNoViolations();
    });

    it("has no accessibility violations when pressed and disabled", async () => {
      const { container } = render(
        <SwitchButton variant="ai" label="Smart replies" defaultPressed disabled />,
      );
      expect(await axe(container)).toHaveNoViolations();
    });
  });
