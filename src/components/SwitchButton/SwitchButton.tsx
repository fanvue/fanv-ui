...
/** Height of the button in pixels. Matches {@link Button}'s scale. */
export type SwitchButtonSize = "24" | "32" | "40";

/** Visual treatment of the button. */
export type SwitchButtonVariant = "default" | "ai";

/** Padding and label typography per size, from `Spacing/Global` xs/sm/md. */
const sizeVariants: Record<SwitchButtonSize, string> = {
...
/** Glyphs are a flat 16px at every size, per the design. */
...
interface SwitchButtonBaseProps
  ...
  label: string;
  /** Height of the button in pixels. @default "32" */
 ...
 */
export interface SwitchButtonDefaultProps extends SwitchButtonBaseProps {
 ...
 */
export interface SwitchButtonAiProps extends SwitchButtonBaseProps {
  /** Visual treatment. */
  variant: "ai";
...
/**
 * Props for {@link SwitchButton}. A discriminated union on `variant`, so the icon
 * slots only type-check on the `"default"` variant.
 */
export type SwitchButtonProps = SwitchButtonDefaultProps | SwitchButtonAiProps;
 ...
 *
 * For two or three mutually exclusive options, reach for {@link SegmentedControl}
 * instead: this button is independently on or off, not a choice between options.
 *
 * @example
 * ```tsx
 * <SwitchButton label="Hide sold out" pressed={hidden} onPressedChange={setHidden} />
 * ```
 *
 * @example Independent icon flags, available on the default variant only
 * ```tsx
 * <SwitchButton size="40" label="Add filter" showLeftIcon showRightIcon />
 * ```
 *
 * @example The AI variant, whose sparkle is fixed
 * ```tsx
 * <SwitchButton variant="ai" label="Smart replies" defaultPressed />
 * ```
 */
