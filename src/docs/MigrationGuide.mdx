...
- [Part 1: Olympus to @fanvue/ui](#part-1-olympus-to-fanvueui)
  ...
  - [Prop Name Changes](#prop-name-changes) — [Button](#button) · [Alert](#alert) · [Avatar](#avatar) · [Tabs](#tabs) · [Switch](#switch-formerly-toggle) · [Checkbox](#checkbox) · [ProgressBar](#progressbar) · [TextField](#textfield) · [Tooltip](#tooltip) · [DatePicker](#datepicker) · [Logo](#logo)
  - [Styling Approach](#styling-approach)
  - [Typography Migration](#typography-migration)
...
- [Part 2: MUI to Tailwind](#part-2-mui-to-tailwind)
  ...
  - [Theme Access: `useTheme` to CSS Variables](#theme-access-usetheme-to-css-variables)
  - [Dark Mode](#dark-mode)
...
| Status | Count |
|---|---|
| ✅ Available in @fanvue/ui (or Tailwind equivalent) | 39 |
...
| `Typography` | N/A (Tailwind utilities) | No component. Use `typography-*` CSS utility classes instead |
...
|---|---|
| `sx={{ padding: 2, backgroundColor: 'red' }}` | `className="p-4 bg-red-500"` |
| `styled(Component)({ ... })` | `className={cn("base-classes", className)}` |
| `theme.spacing(2)` → `16px` | `p-4` (Tailwind `4` = 16px) |
| `theme.palette.brand.green[500]` | `text-brand-green-500` or `bg-brand-green-500` |
| `useTheme()` hook | CSS variables: `var(--color-brand-green-500)` |
...
cn("p-4 text-body-100", isActive && "text-brand-green-500", className);
```
...
> - The `color` prop on Typography (`<Typography color="body.200">`) becomes a Tailwind color class: `text-body-200`.
...
|---|---|:---:|
| **Typography** | 447 | ✅ Tailwind `typography-*` classes |
| **Boxes / Flex** | 270 | ✅ Tailwind `flex` / `grid` |
| **InputFields / TextField** | 87 | ✅ (`TextField` only — `PasswordField`, `TextArea`, `Select` are ❌) |
| **Skeleton** | 73 | ❌ |
...
This section covers the broader migration from MUI's CSS-in-JS approach to Tailwind CSS utility classes — including CSS layer configuration, `sx` prop replacement, `styled()` migration, theme access, and dark mode.
...
/*
 * The Olympus theme applies `padding-left/right: env(safe-area-inset-*)`
 * to `body *` which collides with Tailwind padding utilities. Inside scoped
 * @fanvue/ui regions we revert those properties back to the previous layer
 * so the Tailwind utilities can take effect without touching the global rule.
 */
@layer utilities {
  [data-fanvue-ui],
  [data-fanvue-ui] * {
...
> 1. **MUI's CssBaseline applies global resets** (box-sizing, margin resets, font smoothing). These will conflict with Tailwind's Preflight. During coexistence, keep CssBaseline but be aware it sets `body *` padding via `env(safe-area-inset-*)`. Use the `[data-fanvue-ui]` scoping shown above to revert this for migrated regions.
>
...
// Colors (theme.palette → CSS variables)
sx={{ color: 'body.100' }}                → className="text-body-100"
sx={{ backgroundColor: 'background.200' }} → className="bg-background-200"
sx={{ borderColor: 'neutral.200' }}       → className="border-neutral-200"
...
// After: Tailwind
<div className="rounded-lg bg-background-200 p-4 shadow-sm hover:shadow-md">
  Content
</div>
```
...
const StyledBadge = styled(Box, { shouldForwardProp })<{ $active: boolean }>(
  ({ theme, $active }) => ({
    backgroundColor: $active ? theme.palette.brand.green[500] : theme.palette.neutral[100],
  })
);
...
function Badge({ active, className, ...props }: { active: boolean } & HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        active ? "bg-brand-green-500" : "bg-neutral-100",
        className
      )}
      {...props}
    />
...
### Theme Access: `useTheme` to CSS Variables

> **Scale:** There are **98+ `useTheme` calls** and **1,829 `theme.palette` references** in Eden.

These need to migrate to CSS custom properties.

@fanvue/ui exposes all design tokens as CSS variables on `:root` (light) and `.dark` (dark):

```tsx
// Before: MUI useTheme
const theme = useTheme();
const color = theme.palette.brand.green[500]; // '#49f264ff'
const bgColor = theme.palette.background[200]; // '#ffffffff'

// After: CSS variables (in Tailwind classes)
className="text-brand-green-500 bg-background-200"

// After: CSS variables (in inline styles, if truly dynamic)
style={{ color: 'var(--color-brand-green-500)' }}
```

**Color token mapping:**

| MUI `theme.palette.*` | CSS Variable | Tailwind Class |
|---|---|---|
| `palette.primary[500]` | `--color-primary-500` | `text-primary-500` / `bg-primary-500` |
| `palette.body[100]` | `--color-body-100` | `text-body-100` |
| `palette.brand.green[500]` | `--color-brand-green-500` | `text-brand-green-500` / `bg-brand-green-500` |
| `palette.brand.pink[500]` | `--color-brand-pink-500` | `text-brand-pink-500` |
| `palette.brand.purple[500]` | `--color-brand-purple-500` | `text-brand-purple-500` |
| `palette.background[200]` | `--color-background-200` | `bg-background-200` |
| `palette.neutral[200]` | `--color-neutral-200` | `text-neutral-200` / `bg-neutral-200` |
| `palette.success[500]` | `--color-success-500` | `text-success-500` |
| `palette.warning[500]` | `--color-warning-500` | `text-warning-500` |
| `palette.error[500]` | `--color-error-500` | `text-error-500` |
| `palette.disabled[400]` | `--color-disabled-400` | `text-disabled-400` |
| `palette.hover[100]` | `--color-hover-100` | `text-hover-100` |

**For truly dynamic color needs** (e.g., computing colors in JS), use `getComputedStyle(document.documentElement).getPropertyValue('--color-brand-green-500')` — but prefer Tailwind classes whenever possible.
...
| **@fanvue/ui** | Dark mode uses the `.dark` CSS class on `<html>`. All CSS variables are reassigned under `.dark { ... }` in `theme.css`. Tailwind's `dark:` modifier is configured via `@variant dark (&:where(.dark, .dark *))`. |
...
> - Any `useColorPalette({ oppositeMode: true })` calls need manual handling with CSS variables
...
- `body * { padding-left: env(safe-area-inset-left); padding-right: env(safe-area-inset-right); }` — **This conflicts with Tailwind padding utilities**
- Scrollbar styling for Firefox and Chrome
- `-webkit-tap-highlight-color: transparent`
- `img, video { max-width: 100% }`

Once you have the CSS layer setup (see [CSS Specificity and Layer Ordering](#css-specificity-and-layer-ordering)), the `[data-fanvue-ui]` scoping rule will handle this conflict for @fanvue/ui components. But if you apply Tailwind padding classes to non-scoped elements, the safe-area-inset rule can still override them.

**Workaround during coexistence:** Wrap sections being migrated with `<div data-fanvue-ui>` to opt into the revert-layer fix.
...
MUI's polymorphic `component` prop (`<Box component="section">`, `<Typography component="h2">`) has no equivalent. Use the actual HTML element:
...
// Tailwind
<nav className="p-4">
<h2 className="typography-heading-3">
```
...
### 3. Color Opacity Differences

MUI theme colors in Pandora include alpha channels in the hex value (e.g., `#15151533`). The Tailwind color variables preserve these exact values. However, Tailwind's opacity modifier syntax (`bg-primary-500/50`) will **not work** as expected because the variables already contain alpha. Use the color as-is or use `bg-[color-mix(...)]` for custom opacity.

---

### 4. Transient Props (`$variant`, `$size`) Are Gone
...
### 6. CSS Class Order Doesn't Matter, but Layer Order Does
...
### 7. `useTheme()` in Non-Component Code

If you have utility functions that accept a `theme` parameter:

```tsx
// Before
function getCardStyles(theme: Theme) {
  return { backgroundColor: theme.palette.background[200] };
}

// After — use CSS variables directly
// No function needed; apply className="bg-background-200" in JSX
```

For the rare case where you need the resolved value in JS (e.g., for canvas drawing, chart config), read from CSS:

```tsx
getComputedStyle(document.documentElement).getPropertyValue('--color-background-200');
```

---

### 8. Olympus Theme Constants
...
### 9. Most Form Components Are Not Yet Available
...
### 10. Scoping @fanvue/ui Components During Coexistence

During coexistence, wrap migrated sections with `data-fanvue-ui` to ensure Tailwind padding utilities work correctly despite CssBaseline's safe-area-inset rules:

```tsx
<div data-fanvue-ui>
  {/* @fanvue/ui components and Tailwind-styled elements here */}
...
| 3 | **Typography** → Tailwind utilities | 447 imports | Low (high volume, mechanical) |
| 4 | **IconButton** → @fanvue/ui IconButton | 68 imports | Low |
| 5 | **Alert** → @fanvue/ui Alert | 21 imports | Low |
| 6 | **Chip** → @fanvue/ui Chip | 26 imports | Low |
| 7 | **Avatar** → @fanvue/ui Avatar | 37 imports | Medium (API restructure) |
| 8 | **Checkbox/Radio** → @fanvue/ui | 9 imports | Low |
| 9 | **Toggle** → @fanvue/ui Switch | 7 imports | Low |
| 10 | **sx prop removal** | 2,731 occurrences | High (volume) |
| 11 | **styled() removal** | 127 files | High (volume) |
| 12 | **Loader** → SpinnerIcon | 68 imports | Low |
| 13 | **TextField** → @fanvue/ui TextField | 87 imports | Medium (API changes; PasswordField/TextArea/Select still need MUI) |
| 14 | **Toast** (notistack → Radix) | 7 imports | Medium |
| 15 | **Dialog/Skeleton/Menu** | 119+ imports | Blocked (no @fanvue/ui equivalent) |
